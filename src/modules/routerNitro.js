...
export class Routernitro {
    ...
    async bridgeEth(fromChain, toChain, amountWei, maxFeeWei) {
        ...
        const [ protocolFeeWei, instructions ] = await this.#fetchEthTransaction(fromChainInfo, toChainInfo, amountWei.toString());
        const { txn } = instructions;
        const chainFeeWei = BigInt(txn.gasPrice) * BigInt(txn.gasLimit);
        this.#debugLog(`chainFeeWei ${chainFeeWei}`);

        // Protocol fee safety check
        if (BigInt(protocolFeeWei) > maxFeeWei) {
            throw new Error(`Exceeds max fee: ${formatEther(BigInt(protocolFeeWei))} > ${formatEther(maxFeeWei)}`);
        }
        
        // Tx value safety check
        if (BigInt(txn.value) !== amountWei) {
            throw new Error(`Suspicious transaction value: ${BigInt(txn.value)} initial amount hex: ${amountWei}`);
        }

        // Allowance to address safety check
        ...
        const ethSpent = parseFloat(formatEther(await receipt.gasUsed * await receipt.gasPrice)) + parseFloat(formatEther(BigInt(protocolFeeWei)));
